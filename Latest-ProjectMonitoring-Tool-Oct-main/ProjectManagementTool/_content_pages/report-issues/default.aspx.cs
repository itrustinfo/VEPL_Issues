...
namespace ProjectManagementTool._content_pages.report_issues
{
    public partial class _default : System.Web.UI.Page
    {
        ...
        protected void btnexcelexport_Click(object sender, EventArgs e)
        {
            try
            {
                Guid ExportUID = Guid.NewGuid();
                Guid userUID;
                string Type = "";
                if (DDLUser.SelectedItem.Text.ToString() == "All")
                {
                    userUID = Guid.Empty;
                    Type = "All Users";
                }
                ...
                {
                    userUID = new Guid(DDLUser.SelectedValue);
                    Type = "Single User";
                }
                object userUIDValue = userUID != Guid.Empty ? (object)userUID : DBNull.Value;

                int dsExcel = getdt.InsertExportExcelClick(ExportUID, Type, userUID, new Guid(DDLWorkPackage.SelectedValue));
                DataSet dspath = new DataSet();
                bool ischeck = true;
                string filepath = string.Empty;
                while (ischeck)
                {
                    dspath = getdt.GetExportExcelPath(ExportUID);
                    if (dspath.Tables[0].Rows.Count > 0)
                    {
                        if (dspath.Tables[0].Rows[0]["ExcelCreated"].ToString() == "Y")
                        {
                            ischeck = false;
                            //
                            filepath = dspath.Tables[0].Rows[0]["ExcelPath"].ToString();
                        }
                    }
                    else
                    {
                ...
                //Page.ClientScript.RegisterStartupScript(this.GetType(), "CallMyFunction", "checkbutton2()", true);
            ...
            //    GrdIssues.AllowPaging = false;
            //    LoadIssuesData();
            //    for (int i = 0; i < GrdIssues.Rows.Count; i++)
            //    {
            //        GridViewRow row = GrdIssues.Rows[i];
            //        //Apply text style to each Row
            //        //row.Attributes.Add("class", "textmode");
            //        row.Height = 100;
            //    }
            //    GrdIssues.RenderControl(hw);
        ...
        private void LoadIssuesData()
        {
            ...
            try
            {
                ...
                DataSet ds = getdt.getIssuesList_by_WorkPackageUID_IssueStatus(new Guid(DDLWorkPackage.SelectedValue), status, DDLUser.SelectedValue);
                ...
                {
                    btnExportReportPDF.Visible = false;
                    btnexcelexport2.Visible = true;
                    btnPrintPDF.Visible = false;
                }
                else
                {
                    btnExportReportPDF.Visible = false;
                    btnexcelexport2.Visible = false;
                    btnPrintPDF.Visible = false;
                }
        ...
        {
            DataSet ds = getdt.getAllIssuedUsers(new Guid(DDLWorkPackage.SelectedValue), "");
        ...
        protected void GrdIssues_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                //if (e.Row.RowType == DataControlRowType.DataRow)
                //{
                //    string imageName = "~/_assets/images/cp26.jpg";
                //    string[] filename = imageName.Split('/');

                //    // 70 is define image size.
                //    GenerateThumbNail("~/_assets/images/" + filename[3], "~/_assets/images/cp26_1.jpg", 70);
                //}
                //
        ...
        protected void btnexcelexport2_Click(object sender, EventArgs e)
        {
            try
            {
                Guid ExportUID = Guid.NewGuid();
