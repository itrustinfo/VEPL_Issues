    ...
    public partial class _default : System.Web.UI.Page
    {
            ...
            else
            {
                ...
                if (!IsPostBack)
                {
                    ...
                    if (Session["TypeOfUser"].ToString() == "DDE")
                    {
                        ...
                        DDLUser.SelectedValue = Session["UserUID"].ToString();
                    }
                    // BindIssues(Session["selected_item"].ToString(), TreeView1.SelectedNode.Value, "All");

                    ViewAllIssues.HRef = "/_modal_pages/preview-issue-documents.aspx?ViewAllImages=Yes&UserUID=" + DDLUser.SelectedValue;
                }
                else
                {
        ...
        protected void BindIssues(string IssuesFor, string ActivityUID,string user)
        {
            ...
            if (IssuesFor == "WorkPackage")
            {
                ...
                if (Session["TypeOfUser"].ToString() == "DDE")
                {
                    dataRows = getdt.getIssuesList_by_WorkPackageUID_IssueStatus(new Guid(ActivityUID), status, DDLUser.SelectedValue).Tables[0].AsEnumerable().Where(r =>
                                                                r.Field<string>("Issued_User").ToLower() == Session["UserUID"].ToString().ToLower());
        ...
        protected void GrdIssues_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            {
                if(Session["TypeOfUser"].ToString() != "PM" && Session["TypeOfUser"].ToString() !="U")
                {
                    if(e.Row.Cells[3].Text.ToLower() != Session["UserUID"].ToString().ToLower())
                    {
                        e.Row.Visible = false;
                    }
                }
        ...
        protected void CreateAndDownloadZipFile()
        {
            try
            {
                ...
                {
                    ds1 = getdt.GetAllIssueImages(Guid.NewGuid(), "All");
                }
                else
                {
                    ds1 = getdt.GetAllIssueImages(new Guid(DDLUser.SelectedValue), "NotAll");
                }
                ...
                if (ds1.Tables[0].Rows.Count > 0)
                {
                    foreach (DataRow r in ds1.Tables[0].Rows)
                    {
                        ...
                        string path = Server.MapPath(r.ItemArray[0].ToString());
                        position += 1;

                        if (File.Exists(path))
                        {
                            string outPath = zipFolder + "open_" + r.ItemArray[3].ToString();
                            ...
                            if (files.Where(a => a.Name.Contains(outPath)).Count() == 0)
                                files.Add(new DocFile { Name = outPath, Position = position });
                        }
                    }
                ...
                string zipFilePath = Server.MapPath("/Documents/Issues/" + DDLUser.SelectedItem.ToString() + "__Images.zip");
                ...
                ViewState["zipFilePath"] = zipFilePath;
        ...
        protected void lnldownloadFile_Click(object sender, EventArgs e)
        {
            FileInfo file = new FileInfo(ViewState["zipFilePath"].ToString());
